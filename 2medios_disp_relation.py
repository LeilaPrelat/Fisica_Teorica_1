...
try:
    ...
    from determinante import det_M2medios as det_M
except ModuleNotFoundError as error:
    print(error)
...
for Energy in Elist_NM:
    ...
    def det_M_1variable(Re_kz):
        dett = det_M([Re_kz,im_kz],Energy,modo,R,ε2,ε3)
        return np.abs(dett)
    ...
    plt.title(title +', modo = %i, Im($k_z$) = %.2e, R = %i nm' %(modo,im_kz,R*1e9),fontsize=tamtitle)
...
if (ε2,ε3) == (ε_SiO2,ε_Ag):
    if modo==0:
        ...
        if R==85*1e-9:
            cond_inicial = np.array([165964, -1e8])
    #cond_inicial = np.array([1.17*1e7,66240])
            
    elif modo==1:
        if R==5*1e-9:
            cond_inicial = np.array([203557.97975328687 , 1e4])
...
for Energy in Elist_NM:  
    def det_M_1variable(Re_kz):
        dett = det_M([Re_kz,0],Energy,modo,R,ε2,ε3)
        return np.abs(dett)
